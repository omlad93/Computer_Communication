...
Noise_p generate_noise(Noise_p noise, str noise_type, int a1, int a2) {
    int is_random = strcmp(noise_type, RANDOMIZE);
    int is_deterministic = strcmp(noise_type, DETERMINISTIC);
...
void apply_randomised(Noise_p noise, str data, int size) {
    int flipping;
    for (int i = 0; i < size; i++) {
...
int apply_noise(Noise_p noise, str data, int size) {
    if (noise->type == RANDOMIZE) {
        apply_randomised(noise, data, size);
    } else {
...
int main(int argc, char* argv[]) {
    ...
    assert_num((argc <= 3) & (argc > 1), "Noisy Channel Got Unexpected Numer og Arguments", argc);
    noise_type = argv[1];
    a1 = atoi(argv[2]);
    ...
    create_address(&server_sa, server_port, server_ip);
    FD_ZERO(&fs);
    while (True) {
